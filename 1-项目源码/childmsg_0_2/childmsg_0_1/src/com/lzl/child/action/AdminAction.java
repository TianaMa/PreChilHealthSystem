			...
			if(method.equals("one")){//admin登录
				...
				if(username == null||username.trim().equals("")){
					...
					switch (flag){
						case Constant.SUCCESS:
							...
							request.getRequestDispatcher(sysdir+"/index.jsp").forward(request, response);
							break;
				...
				if(username2 == null){
					request.getRequestDispatcher("error.jsp").forward(request, response);
						...
						String fudaoyuan = request.getParameter("fudaoyuan").trim();
						
						int flag = loginbean.addManager(username, password, "2", "1",fudaoyuan);
