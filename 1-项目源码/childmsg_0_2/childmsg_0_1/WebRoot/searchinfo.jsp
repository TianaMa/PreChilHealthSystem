<%@ page language="java" import="java.util.*,com.lzl.child.util.*" contentType="text/html;charset=gb2312" %>
...
<%
	...
	String myname=Common.toChineseAndTrim(request.getParameter("myname"));
	System.out.println("..."+title+"-----"+fl+"...."+myname);
	String sql = "select * from zt where fl = '"+fl+"' and  title like '%"+title+"%' and title like '%"+myname+"%' order by id desc";
...
<TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
      ...
      <td  align="center">点击</td> 
                    ...
                    <%
                    List hotlist=cb.getCom(sql,7);
                    if(!hotlist.isEmpty()){
					for(int alln1=0;alln1<hotlist.size();alln1++){
						List allnew=(List)hotlist.get(alln1);
			%>
        <tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
       <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath %>ztinfo.jsp?id=<%=allnew.get(0).toString()%>" target="_blank"><%=allnew.get(1).toString() %></a> </td>
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=allnew.get(3).toString() %></td>
       <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=allnew.get(5).toString() %></td>
       <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=allnew.get(6).toString() %></td>
       </tr>
		<%}} %>
				</TBODY>
			</TABLE>			
