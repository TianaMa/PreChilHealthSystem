     ...
     <TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		<TD  class=middle align="left" >
               <TABLE class=xsnr id=tb1_nr1  cellSpacing=0 cellPadding=0 width="100%" border=0>
                    <TBODY>
                    <%
                    String id=request.getParameter("id").trim();cb.comUp("update zt set dj=dj+1 where id='"+id+"'");
                    List hotlist=cb.get1Com("select * from zt where id='"+id+"'",7);
                    if(!hotlist.isEmpty()){
         			%>
         <TR>
          <TD align=center><strong><%=hotlist.get(1).toString() %></strong></TD>
		</TR>
        <TR>
          <TD align=center>分类： <%=hotlist.get(3).toString() %> 发布时间：<%=hotlist.get(5).toString() %> 点击：<%=hotlist.get(6).toString() %> 
          <a href="<%=basePath%>down.jsp?url=<%=hotlist.get(2).toString()%>">下载文章</a></TD>
		</TR>
		<TR>
          <TD align=center><%=hotlist.get(4).toString() %></TD>
		</TR>
		<%}%>
