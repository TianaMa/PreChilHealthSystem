...
<script  language="javascript" >
function top(){
   	form3.action="<%=basePath%>pc.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%>pc.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>pc.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>pc.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>pc.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>pc.jsp?page="+page;
      form3.submit();
...
//-->
     ...
     <TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
      ...
      <td  align="center">答题时间</td>
      <td  align="center">查看评分</td> 
      <td  align="center">选择</td>
    </tr>
                    <%cb.setEVERYPAGENUM(12);
			int cou = cb.getMessageCount("select count(*) from sjmc");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sjmc order by id desc",3);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <a href="<%=basePath%>pf2.jsp?sjid=<%=pagelist2.get(0).toString()%>">查看</a></TD>
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;">
      <a href="<%=basePath%>dt.jsp?sjid=<%=pagelist2.get(0).toString()%>">答题</a></TD>
    </tr>
<%
	}}
%>  
